...
SELECT BIN_TO_UUID(id) AS card_uuid
FROM cards
WHERE name = 'nombre_tarjeta' AND list_id = UUID_TO_BIN('list_uuid');

-- GET THE UUID OF A USER BY ITS NAME --
SELECT BIN_TO_UUID(id) AS user_uuid
FROM users
...
WHERE com.card_id = UUID_TO_BIN('card_uuid')
...
WHERE ca.user_id = UUID_TO_BIN('user_uuid')
...
-- GAT THE USERS WITH THE MOST COMMENTS MADE --
